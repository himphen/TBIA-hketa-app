...
open class GeoHash {
    ...
    private val numberOfLatLonBits
        get() = intArrayOf(
            significantBits / 2,
            significantBits / 2 + if (significantBits % 2 == 0) 0 else 1
        )
    ...
    private val rightAlignedLatitudeBits: LongArray
        get() {
            ...
            return longArrayOf(
                extractEverySecondBit(bitsCopy, numberOfLatLonBits[0]),
                numberOfLatLonBits[0].toLong()
            )
        }
    ...
    private val rightAlignedLongitudeBits: LongArray
        get() {
            val bitsCopy = bits
            return longArrayOf(
                extractEverySecondBit(bitsCopy, numberOfLatLonBits[1]),
                numberOfLatLonBits[1].toLong()
            )
        }
    ...
    fun next(step: Int) =
        GeoHash((ord + step) shl MAX_BIT_PRECISION - significantBits, significantBits)
    ...
    private fun generateLocation(lat: Double, lon: Double) = Location().also {
        it.latitude = lat
        it.longitude = lon
    }
